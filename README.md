...
You simply need to download the [zip file](https://github.com/smirarab/ASTRAL/raw/master/Astral.4.10.14.zip) 
...
```
java -jar astral.4.10.14.jar -i test_data/song_primates.424.gene.tre
```

This should quickly finish. There are also other sample input files under `test_data/` that can be used.

ASTRAL can be run from any directories. You just need to run `java -jar /path/to/astral/astral.4.10.14.jar`.
Also, you can move `astral.4.10.14.jar` to any location you like and run it from there, but note that you need
...
```
java -jar astral.4.10.14.jar -i in.tree
```
...
```
java -jar astral.4.10.14.jar -i in.tree -o out.tre
```
...
```
java -jar astral.4.10.14.jar -i best_ml -b bs_paths -r 100
```
